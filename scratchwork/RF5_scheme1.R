...
for(i in 1:itrExp){#for begins
  ...
  # cat(i, "th LLE", "\n")
  # LLEout[[i]] <- fn4_embdRf(trnX, trnY, numVar = 2, teX, teY, embdMethod = "LLE", numTrees = 30)
}
...
#Export the results
setwd("C:\\Users\\H.Kim\\Google Drive\\RF\\shceme1")

save(PCAout, file = "PCAout.rda")
...
#
#input
#@dataList: subsetList: A list of feature subsets
#@bootstrapRate: bootstrapping rate
#output
#@ls_bootSubsets: A list of bootstrapped subsets
#########################################################
fn2_bootstrap = function(dataList, bootstrapRate){
  ls_bootIdx <- list()
  ls_bootSubsets <- list()
  
  i <- 1
  for(i in 1:length(dataList)){#for begins
    #bootstrap sampling
    numRecord <- nrow(data.frame(dataList[[i]]))
    ls_bootIdx[[i]] <- sample(1:numRecord, round(bootstrapRate * numRecord), replace = TRUE)
    
    #save a bootstrapped subset in the 'ls_bootSubsets'
    ls_bootSubsets[[i]] <- data.frame(dataList[[i]][ls_bootIdx[[i]], ])
    names(ls_bootSubsets)[i] <- paste0("bootstrap", i)
  }#for ends
  
  return(ls_bootSubsets)   
}
...
#########################################################
#function 3
#Basic rotation forest
#########################################################
fn_PCArf = function(trnX, trnY, numVar = 2, bootstrapRate = 0.75, teX, teY, numTrees){
  ...
  for(iTree in 1:numTrees){#for begins
    ...
    #Get feature subsets
    ls_subsets <- fn1_getRandomSubset(x = trnX, numVar)
    
    #Get bootstrapped subsets
    ls_boots <- fn2_bootstrap(ls_subsets, bootstrapRate)
    
    #Get principal components of each subsets
    PCAcomp <- lapply(ls_boots, function(x){prcomp(x, center = TRUE)$rotation})
    
    #Arrangement if the principal components
    arrgPCs <- as.vector(unlist(lapply(PCAcomp, rownames)))
    
    #'bdiag' is a function in the Matrix package
    #It returns block-shaped diagonal matrix which is sparse
    rotationMx <- as.matrix(do.call(bdiag, lapply(PCAcomp, as.matrix)))
    rownames(rotationMx) <- arrgPCs
    
    #Rearrage the sequecnce of row vectors
    rotationMx1 <- rotationMx[order(arrgPCs), ]
    ...
    #Set rotated training data
    xRy <- data.frame((as.matrix(trnX) %*% rotationMx1), Y = trnY)
    #ls_xRy[[i]] <- xRy
    
    #Fit a tree model
    oneTree <- rpart(Y ~., method = "class", data = xRy)
    #ls_tree[[i]] <- oneTree
    
    #Prepare rotated test data
    colnames(teX) <- paste0("X", 1:length(teX)) 
    newTestX <- data.frame(as.matrix(teX) %*% rotationMx1)
  ...
  #Make a prediction
  prdt <- factor(apply(aggRlt, 1, function(x){
    colnames(aggRlt)[which(x == max(x))]}), levels = colnames(aggRlt))
  
  #Generate a confusion matrix
  confMx <- confusionMatrix(prdt, teY)
...
#########################################################
#function 4: Nonlinear rotation forest 
#########################################################
    ...
    switch (dimRdcMethod,
            LLE = {#LLE begins
              ls_LLE <- lapply(ls_trnX, function(x){
                lle(x, m = length(x), k = 15, reg = 2, id = TRUE)})
